...
class App:
    ...
    def __init__(self):
        ...
        import logging
        import napari
        from segmentation import SegmentCytoplasm

        logging.basicConfig(level=logging.DEBUG)

        # Assigning the class Dataset to the variable d1 /KM
        d1 = Dataset(name="lumbar_1")
        # Loading Nuclei and Cytoplasm images and storing(is it storing them?)
        # them in d1 /KM
        d1.load_nuclei()
        d1.load_cytoplasm()
        # d1.load_other_channel(channel="green")
        d1.load_gene_expression()
        # Assigning the gene expression information in variable
        # df/dataframe(?) /KM
        df = d1.gene_expression

        # Creating a list called idx /KM
        idx = list()
        # For loop, goes through the genes in each row of our data
        # and all genes that fit the coordinates critera will
        # be put into list idx /KM
        for i, gene in df.iterrows():
            if gene.x > 6000 and gene.x <= 8000:
                if gene.y > 5000 and gene.y <= 6000:
                    idx.append(i)

        # iloc lets us choose a specific location/cell in our list.
        # Subtracting specific values from the X and Y coordinates /KM
        df = df.iloc[idx].copy()
        df.x = df.x - 6000
        df.y = df.y - 5000
        d1.gene_expression = df

        # Altering our cytoplasm and nuclei images to the same coordinates
        # that we have in our gene list /KM
        d1.images["Cytoplasm"] = d1.images["Cytoplasm"][5000:6000, 6000:8000]

        d1.images["Nuclei"] = d1.images["Nuclei"][5000:6000, 6000:8000]

        # Running the segmentation process which is defined in the
        # segmentation.py code, setting our own values /KM
        ...
        # Choosing Napari as the program we want to look in, labeling
        # our axis, adding our images and assigning color to them.
        # The genes are added as dots and the gene name is added as
        #  text /KM
